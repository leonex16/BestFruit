...
CREATE OR REPLACE PROCEDURE SP_SALDOS(SP_SALDOS OUT SYS_REFCURSOR) IS
    ...
    FROM
        STOCK_SOBRANTE STOCK
            ...
            RIGHT JOIN DETALLE_OFERTA DETA_OFER
                ON DETA_OFER.ID_PEDIDO = (
                                            SELECT ID_PEDIDO FROM DETALLE_OFERTA WHERE CERRADA = 1
                                         )
    ...
    WHERE
        (VENTA.ID_ESTADO = 1 OR VENTA.ID_ESTADO IS NULL) AND DETA_OFER.CERRADA = 1
    ORDER BY
        VARIEDAD.DES_VARIEDAD;
